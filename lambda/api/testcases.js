...
const TABLE_NAME = process.env.TESTCASES_TABLE;
    ...
    try {
        switch (method) {
            case 'GET':
                ...
                if (queryParams.agentId) return await getByAgent(queryParams.agentId);
                return await getAll();
            case 'POST':
...
async function getByAgent(agentId) {
    const result = await docClient.send(new QueryCommand({
        TableName: TABLE_NAME,
